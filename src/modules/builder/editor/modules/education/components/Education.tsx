  ...
  return (
    <Fragment>
      ...
      <div className="mt-2">
        ...
        <Stack spacing={2}>
          {(educationInfo.results ?? []).map((res, idx) => (
            <Stack key={idx} direction="row" spacing={1} alignItems="center">
              <TextField
                ...
                value={res.subject}
                onChange={(e: ChangeEvent<HTMLInputElement>) => {
                  const updated = [...(educationInfo.results ?? [])];
                  updated[idx] = { ...updated[idx], subject: e.target.value };
                  onChangeHandler('results', updated);
                }}
                fullWidth
              />
              <TextField
              ...
              <Button
                ...
                color="error"
                onClick={() => {
                  const updated = (educationInfo.results ?? []).filter((_, i) => i !== idx);
                  onChangeHandler('results', updated);
                }}
              >
                Remove
